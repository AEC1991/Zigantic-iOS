...
public class SlidingContainerSliderView: UIScrollView, UIScrollViewDelegate {
  ...
  public init(width: CGFloat, titles: [String]) {
    ...
    appearance = SlidingContainerSliderViewAppearance (
      ...
      selectedFont: UIFont (name: "Open Sans", size: 14)!,
  ...
  public func draw() {
    ...
    if appearance.fixedWidth {
      ...
      for title in titles {
        ...
        labelTag += 1
        label.tag = labelTag
        let view = UIView(frame: CGRect(x: label.frame.maxX, y: 0, width: 0.5, height: self.frame.size.height))
        view.backgroundColor = .lightGray
        let viewdown = UIView(frame: CGRect(x: label.frame.minX, y: self.frame.size.height-0.5, width: label.frame.size.width, height: 0.5))
        viewdown.backgroundColor = .lightGray
        addSubview(viewdown)
        addSubview(view)
        addSubview(label)
        labels.append(label)
